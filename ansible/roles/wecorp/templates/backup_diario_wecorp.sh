...
tar -cvzf  $BACKUP_DIR/bkdiario{{ dbname }}`date +%Y%m%d`.tar.gz /servicios/{{ dbname }} --exclude '/servicios/{{ dbname }}/logs' --exclude '/servicios/{{ dbname }}/tomcat-{{ file }}/logs' --exclude '/servicios/{{ dbname }}/tomcat-{{ file }}/temp/*' --exclude '/servicios/{{ dbname }}/tomcat-{{ file }}/work/Catalina' |grep -v "File removed before we read it"  && monitor="OK"
...
old_output_file="$BACKUP_DIR/bkdiario{{ dbname }}$(date --date='10 days ago' +'%Y%m%d').tar.gz"
